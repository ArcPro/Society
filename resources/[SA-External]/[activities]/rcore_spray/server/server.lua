...
Citizen.CreateThread(function()
    if Config.SprayCooldownTime == nil then
        Config.SprayCooldownTime = 1
    end
    if Config.SprayCooldownTotal == nil then
        Config.SprayCooldownTotal = -1
    end

    if Config.Text.SPRAY_LIMIT_COOLDOWN == nil then
        Config.Text.SPRAY_LIMIT_COOLDOWN = 'You exceeded the amount of allowed graffiti within a time period. Try again later.'
    end
...
function AddSpray(Source, spray)
    ...
    if spray.identifier and type(spray.identifier) == 'string' and string.len(spray.identifier) > 4 then
        if not SprayCooldown[spray.identifier] then
            SprayCooldown[spray.identifier] = 0
        end

        SprayCooldown[spray.identifier] = SprayCooldown[spray.identifier] + 1

        Citizen.SetTimeout(Config.SprayCooldownTime * 1000, function()
            SprayCooldown[spray.identifier] = SprayCooldown[spray.identifier] - 1

            if SprayCooldown[spray.identifier] <= 0 then
                SprayCooldown[spray.identifier] = nil
            end
        end)
    end
...
function StartSpraying(source)
    ...
    if Config.SprayCooldownTotal > 0 then
        if ident and type(ident) == 'string' and string.len(ident) > 4 then
            if SprayCooldown[ident] and SprayCooldown[ident] >= Config.SprayCooldownTotal then
                ShowNotification(source, Config.Text.SPRAY_LIMIT_COOLDOWN)
                return
            end
        end
    end
...
RegisterNetEvent("rcore_spray:startSpray")
AddEventHandler('rcore_spray:startSpray', function()
	local src = source
	StartSpraying(src)
end)

RegisterNetEvent("rcore_spray:removeSpray")
AddEventHandler('rcore_spray:removeSpray', function()
	local src = source
