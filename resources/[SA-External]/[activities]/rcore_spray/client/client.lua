...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for _, spray in ipairs(SPRAYS) do
            ...
            if dist < (Config.SprayRenderDistance or 100.0) then
...
function RenderSortedSprays(coords, s)
    ...
    local finalCamPos, leftProng, rightProng = ComputeGameplayFrustrumParams()

    for i = 1, math.min(#s, scaleformBudget) do
        local ptr = s[i][2]
        local sprayLoc = ptr.realLocation

        if not loc then
            sprayLoc = ptr.location
        end

        if not FrustumCullSprays or IsInFrustrum(finalCamPos, leftProng, rightProng, sprayLoc) then
            DrawSpray(PLAYER_NAME_HEAP[scaleformHandleIdx], ptr)
