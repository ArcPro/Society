...
function useVehicleWigwag(veh)
	local model = GetEntityModel(veh)
	for i = 1, #eModelsWigwag, 1 do
		if model == GetHashKey(eModelsWigwag[i]) then
			return true
		end
	end
	return false
						...
						if not key_lock then
							...
							if IsDisabledControlJustReleased(0, 85) then
								if lights_on then
									...
									if useVehicleWigwag(veh) then
										print("on")
										SetVehicleAutoRepairDisabled(veh, true)
										if GetEntityModel(veh) == '-1392875269' then
