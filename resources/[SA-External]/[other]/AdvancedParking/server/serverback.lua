...
if (not GetEntityFromStateBagName) then
    LogDebug(
        "Function \"GetEntityFromStateBagName\" was not detected. Adding custom implementation...")
...
function CreateAndReadFromDB()
    ...
    MySQL.Sync.execute("CREATE TABLE IF NOT EXISTS `vehicle_parking` (" ..
    ...
    if (not hasTypeColumn) then
        MySQL.Sync.execute("ALTER TABLE `vehicle_parking` " ..
                               "\nADD COLUMN `type` varchar(16)")
    end

    local rows = MySQL.Sync.fetchAll(
                     "SELECT vp.id, v.plate, v.cover_coords, v.owner, vp.model, vp.type, vp.status, vp.tuning, vp.stateBags, vp.posX, vp.posY, vp.posZ, vp.rotX, vp.rotY, vp.rotZ, vp.lastUpdate FROM vehicle_parking vp INNER JOIN vehicles v ON v.plate = JSON_UNQUOTE(JSON_EXTRACT(vp.tuning, '$[0]')) WHERE v.owner != 'Inconnu' AND v.cover_coords != '[]'")
    ...
    for i, row in ipairs(rows) do
        ...
        if (row.model ~= nil and tune[1] ~= nil) then
            ...
            LogWarning(
                "Vehicle \"%s\" does not have a model or plate associated with it. Deleting...",
                row.id)
...
function CleanUp()
    ...
    for id, vehicleData in pairs(savedVehicles) do
        if (vehicleData.lastUpdate < timeDiff or
            (Config.cleanUpExplodedVehicles and vehicleData.status[3] < -3999.0)) then
            toDelete[#toDelete + 1] = id
        end
    end

    for i, id in ipairs(toDelete) do
        if (savedVehicles[id].handle and
            DoesEntityExist(savedVehicles[id].handle)) then
        ...
        if (Config.storeOwnedVehiclesInGarage) then
            ...
            local tableName = isESX and "owned_vehicles" or "vehicles"
            local colName = isESX and "stored" or "status"

            local plate = savedVehicles[id].tuning[1]:upper()
            MySQL.Async.execute(
                "UPDATE `" .. tableName .. "` SET `" .. colName .. "` = 1 " ..
                    "\nWHERE `plate` = @plate OR `plate` = @trimmedPlate",
                {['@plate'] = plate, ['@trimmedPlate'] = Trim(plate)})
        end
    ...
    for id, vehicleData in pairs(savedVehicles) do
        if (not vehicleData.spawning and vehicleData.handle == nil) then
            ...
            if (loadedVehicle and DoesEntityExist(loadedVehicle)) then
                ...
                LogDebug("Found vehicle \"%s\" at %s", id,
                         RoundVector3(GetEntityCoords(vehicleData.handle), 2))
            elseif (GetClosestPlayer(vehicleData.position, SPAWN_DISTANCE,
                                     players, playerPedsWithHandlers,
                                     vehicleData.stateBags.bucket)) then
...
function SpawnVehicle(id, vehicleData)
    ...
    vehicleData.handle = vehicle
		...
		local coords = vector4(vehicleData.position.x, vehicleData.position.y, vehicleData.position.z, heading)
		Outro:coverVehicle(vehicleData.tuning[1], coords, vehicle)
...
function UpdateVehicles(players, playerPedsWithHandlers, playerPeds)
    for id, vehicleData in pairs(savedVehicles) do
        if (vehicleData.handle and DoesEntityExist(vehicleData.handle) and
            GetClosestPlayer(vehicleData.position, 150.0, players, playerPedsWithHandlers, GetEntityRoutingBucket(vehicleData.handle)) and
            NetworkGetEntityOwner(vehicleData.handle) ~= -1) then
...
function TryUpdateVehicle(id, vehicleData, playerPeds)
    ...
    if (IsAnyPlayerInsideVehicle(handle, playerPeds) or
        Entity(handle).state.isAttached) then return end
    ...
    if ((newLockStatus == 0 or newLockStatus == 1) and
        (status[7] == 0 or status[7] == 1)) then
    ...
    if (posChange or rotChange or lockChange or engineHealthChange or
        tankHealthChange) then
        LogDebug("Reason for next update:")
        if (posChange) then
            LogDebug("    - Position from %s to %s", vehicleData.position,
                     newPos)
        end
        if (rotChange) then
            LogDebug("    - Rotation from %s to %s", vehicleData.rotation,
                     newRot)
        end
        if (lockChange) then
            LogDebug("    - Lockstate from %s to %s", status[7], newLockStatus)
        end
        if (engineHealthChange) then
            LogDebug("    - Engine from %s to %s %s", status[3], newEngineHealth)
        end
        if (tankHealthChange) then
            LogDebug("    - Tank from %s to %s %s", status[4], newTankHealth)
        end
        ...
        if tankHealthChange then vehicleData.status[4] = newTankHealth end

        if lockChange then vehicleData.status[7] = newLockStatus end
...
RegisterNetEvent("AP:updateVehicle", function(networkId, model, tuning, status)
    ...
    if (not DoesEntityExist(vehicle)) then
        LogDebug(
            "Tried to save entity that does not exist on server side (yet)!")
        return
    end
    ...
    if (id and savedVehicles[id]) then
        ...
        if (Config.preventDuplicateVehicles) then
            ...
            if (oldId) then DeleteVehicleUsingIdentifier(oldId) end
        end
...
function InsertVehicleInDB(id)
    assert(id ~= nil and type(id) == "string",
    ...
    MySQL.Async.execute(
            ...
            "\nVALUES (@id, @model, @type, @status, @tuning, @stateBags, @posX, @posY, @posZ, @rotX, @rotY, @rotZ, @lastUpdate)",
        {
            ...
            ["@stateBags"] = json.encode(savedVehicles[id].stateBags,
                                         {indent = true}),
...
function UpdateVehicleInDB(id, vehicleData)
    ...
    MySQL.Async.execute("UPDATE `vehicle_parking` " ..
                            "\nSET `status` = @status, " ..
                            "\n`tuning` = @tuning, " ..
                            "\n`stateBags` = @stateBags, " ..
...
function DeleteVehiclesFromDB(...)
    ...
    if (type(idList[1]) == "table") then idList = idList[1] end
...
AddStateBagChangeHandler("ap_spawned", "",
    ...
    if (DoesEntityExist(savedVehicles[id].handle)) then
        ...
        TriggerClientEvent("AP:vehicleSpawned", -1,
...
Citizen.CreateThread(function()
    while (true) do
        ...
        Citizen.CreateThread(function() DeleteAllVehicles() end)
    end
end)
...
function DeleteAllVehicles()
    ...
    for i, vehicle in ipairs(vehicles) do
        if (DoesEntityExist(vehicle) and
            CountPlayersInBucket(Entity(vehicle).state.bucket or 1) == 0) then
            if Entity(vehicle).state.clamp then
                DeleteEntity(NetworkGetEntityFromNetworkId(
                                 Entity(vehicle).state.clamp))
            end
...
function DeleteVehicle(vehicle, keepInWorld)
    ...
    DeleteVehicleUsingData(Entity(vehicle).state.ap_id,
                           NetworkGetNetworkIdFromEntity(vehicle),
                           GetVehicleNumberPlateText(vehicle), keepInWorld)
end
...
function DeleteVehicleUsingData(identifier, networkId, plate, keepInWorld)
    if (identifier == nil and networkId == nil and plate == nil) then
        LogWarning(
            "Tried to delete vehicle without \"identifier\", \"networkId\" and \"plate\"!")
        return
    end

    LogDebug(
        "Trying to delete using identifier \"%s\", networkId \"%s\", plate \"%s\"",
        identifier, networkId, plate)
    ...
    if (plate and DeleteVehicleUsingPlate(plate, keepInWorld)) then return end
end
...
RegisterNetEvent("AP:deleteVehicle",
                 function(identifier, networkId, plate, keepInWorld)
...
function DeleteVehicleUsingIdentifier(id, keepInWorld)
    ...
    if (savedVehicles[id]) then
        if (not keepInWorld and savedVehicles[id].handle and
            DoesEntityExist(savedVehicles[id].handle)) then
            DeleteEntity(savedVehicles[id].handle)
        end
        ...
        if (not result) then
            LogError(
                "Error occured while calling \"DeleteVehiclesFromDB\" inside \"DeleteVehicleUsingIdentifier\"!")
...
function DeleteVehicleUsingNetworkId(networkId, keepInWorld)
    ...
    for id, vehicleData in pairs(savedVehicles) do
        if (vehicleData.handle == vehicle) then
            if (not keepInWorld) then DeleteEntity(vehicle) end
...
function DeleteVehicleUsingPlate(plate, keepInWorld)
    ...
    for id, vehicleData in pairs(savedVehicles) do
        if (vehicleData.tuning[1] == plate or Trim(vehicleData.tuning[1]) ==
            plate) then
            if (not keepInWorld and vehicleData.handle and
                DoesEntityExist(vehicleData.handle)) then
                DeleteEntity(vehicleData.handle)
            end
...
AddStateBagChangeHandler("", "",
    ...
    if (savedVehicles[id].stateBags[key] == nil or
        not TableEquals(savedVehicles[id].stateBags[key], value)) then
        ...
        if (Config.preventStateBagAutoUpdate) then
            for i = 1, #Config.preventStateBagAutoUpdate do
                if (key:find(Config.preventStateBagAutoUpdate[i])) then
                    return
                end
            end
        end
        ...
        LogDebug("    Reason: Updating entity state bag \"%s\" for \"%s\"", key,
                 id)
    end
end)
...
function GetVehiclePosition(plate)
    assert(plate ~= nil and type(plate) == "string",
           "Parameter \"plate\" must be a string!")
    ...
    for id, vehicleData in pairs(savedVehicles) do
        if (vehicleData.tuning and
            (plate == vehicleData.tuning[1] or plate ==
                Trim(vehicleData.tuning[1]))) then
...
function GetVehiclePositions(plates)
    for i, plate in ipairs(plates) do
        assert(plate ~= nil and type(plate) == "string",
               "Parameter \"plates\" must only contain strings!")
        plate = plate:upper()
    end
        ...
        if (DoesEntityExist(vehicle)) then
            ...
            for j, plate in ipairs(plates) do
                if (plate == vehPlate or plate == trimmedVehPlate) then
                    platePositions[plate] = RoundVector3(
                                                GetEntityCoords(vehicle), 2)
    ...
    for i, plate in ipairs(plates) do
        if (platePositions[plate] == nil) then
            for id, vehicleData in pairs(savedVehicles) do
                ...
                if (vehicleData.tuning and
                    (plate == vehicleData.tuning[1] or plate == trimmedVehPlate)) then
...
CB:Register("AP:getVehiclePosition",
            function(source, plate) return GetVehiclePosition(plate) end)
CB:Register("AP:getVehiclePositions",
            function(source, plates) return GetVehiclePositions(plates) end)
...
RegisterCommand("deleteSavedVehicles", function(source, args, raw)
    if (deleteSavedVehicles) then
        ...
        Log(
            "Are you sure that you want to delete all vehicles from the parking list?\nIf yes, execute the command a second time!")
    end
...
function UpdatePlate(networkId, newPlate, oldPlate)
    ...
    if (newPlate == nil or newPlate:len() > 8) then
        LogError(
            "\"newPlate\" was nil or too long while trying to update a plate!")
        return
    end

    -- format plates
    newPlate = Trim(newPlate:upper())
    if (oldPlate) then oldPlate = Trim(oldPlate:upper()) end
    ...
    for id, vehicleData in pairs(savedVehicles) do
        if (vehicle == vehicleData.handle) then
            ...
            LogDebug("    Reason: Updating plate from \"%s\" to \"%s\"", old,
                     newPlate)
    ...
    if (oldPlate) then
        ...
        for id, vehicleData in pairs(vehicles) do
            if (Trim(vehicleData.tuning[1]) == oldPlate) then
                ...
                LogDebug("    Reason: Updating plate from \"%s\" to \"%s\"",
                         oldPlate, newPlate)
