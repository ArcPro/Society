...
function IsAnyPlayerInsideVehicle(vehicle, playerPeds)
	...
	for i = 1, #playerPeds do
		if DoesEntityExist(playerPeds[i]) then
...
function GetClosestPlayer(position, maxRadius, players, playerPedsWithHandles, bucket)
	...
	for i = 1, #players do
		if (GetPlayerRoutingBucket(players[i]) == (bucket or 1)) then
...
function CountPlayersInBucket(bucket)
	local count = 0
	...
	for k, source in pairs(GetPlayers()) do
		if GetPlayerRoutingBucket(source) == bucket then
			count += 1
		end
	end
