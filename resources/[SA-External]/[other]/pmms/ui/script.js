...
var currentServerEndpoint = '51.254.38.95:30120';
	...
	if (data.uiIsOpen) {
		...
		if ((!data.permissions.interact || usableMediaPlayers.length == 0) && !scaleformCheckbox.checked) {
			...
			usableMediaPlayers.forEach(mediaPlayer => {
				...
				// If not handle, get the nearest usable media player
				const nearest = Math.min(...usableMediaPlayers.map(mp => mp.distance));

				if (!usableMediaPlayersValue && mediaPlayer.distance == nearest) {
					option.selected = true;
				}

				if (!usableMediaPlayersValue && mediaPlayer.handle == selectedEntity) {
					option.selected = true;
				}
...
function hideUi() {
	...
	// Remove all media players select
	var usableMediaPlayersSelect = document.getElementById('usable-media-players');
	usableMediaPlayersSelect.innerHTML = '<option></option>';
}
