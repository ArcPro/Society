...
local function startMediaPlayer(handle, options)
	...
	elseif doesEntityExist(handle) then
		...
		options.bucket = exports.SidGamemode:getRoutingBucket()
...
AddEventHandler("pmms:sync", function(players)
	if syncIsEnabled then
		...
		for handle, info in pairs(mediaPlayers) do
			if info.bucket ~= exports.SidGamemode:getRoutingBucket() then
...
exports("IsMediaPlayableProp", function(entity)
	if not entity then
		return false
	end
	...
	return isMediaPlayer(entity)
end)
exports("OpenUIForEntity", OpenUIForEntity)
