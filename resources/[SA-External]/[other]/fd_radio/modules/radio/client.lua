...
function joinRadio(data, cb)
    ...
    if Config.EncryptedChannels[math.floor(data.radio)] ~= nil then
        local PlayerJob = bridge.getJob()

        if not findInTable(Config.EncryptedChannels[math.floor(data.radio)].jobs, PlayerJob) then
            return bridge.notify(Locale.restricted_channel_error, 'error')
        end
    end
...
CreateThread(function()
    while true do
        ...
        if isOpen and not bridge.hasItem(Config.UseItemName, 1) then
            close()
            ui.radioToggle(false)
        end
